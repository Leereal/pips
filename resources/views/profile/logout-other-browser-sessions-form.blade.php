...
<x-jet-action-section>
    ...
    <x-slot name="content">
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            <x-slot name="title">
                <h3 class="text-{{$text}}">{{ __('Log Out Other Browser Sessions') }}</h3>
                
            </x-slot>

            <x-slot name="content">
                <p class="text-{{$text}}">{{ __('Please enter your password to confirm you would like to log out of your other browser sessions across all of your devices.') }}</p>
                

                <div class="mt-4" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" class="d-block form-control bg-{{$bg}} text-{{$text}}"
